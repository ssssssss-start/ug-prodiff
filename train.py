...
class Diffusion(nn.Module):
    ...
    def p_sample(self, img_noise, t, clip_denoised=True, condition_hrMS=None, condition_lrHS=None,SRF=None):
        ...
        mean, log_variance = self.p_mean_variance(x=img_noise, t=t, clip_denoised=clip_denoised, condition_hrMS=condition_hrMS, condition_lrHS=condition_lrHS,SRF=SRF)
    ...
    def super_resolution(self, gtHS, hrMS, lrHS,SRF):
        img = torch.rand_like(gtHS, device=gtHS.device)
        lrHS_up = self.upsample(lrHS)
        # 粗配准
        ...
        for i in range(start_epoch, epoch):
            ...
            if train:
                for step, [gtHS, hrMS, lrHS] in enumerate(tqdm(self.dataloader)):
                    ...
                    loss = self.sr3(gtHS, hrMS, lrHS,SRF)
            ...
            if (i + 1) % verbose == 0:
                ...
                fuse_result = self.test(gtHS.unsqueeze(0), lrHS.unsqueeze(0), hrMS.unsqueeze(0),self.SRF)
                ...
                Metric = iv.spectra_metric(pred01 , gt01, 1, 4)  # max_v/l=1, scale=4
                PSNR = Metric.PSNR()
                SAM = Metric.SAM()
                SSIM = Metric.SSIM()
                MSE = Metric.MSE()
                ...
                print(y)
                # 定义 TXT 文件路径
                txt_file = "/disk3/sqy/model/Houston/LRHS_Elastic1000_C1/evaluation_metrics.txt"

                # 写入数据
                with open(txt_file, "a") as file:
                    file.write(f"Index: {i}, PSNR: {PSNR}, SAM: {SAM}, SSIM: {SSIM}, MSE: {MSE}, ERGAS: {ERGAS}\n")

    def test(self, gtHS, lrHS, hrMS,SRF):
        lrHS = lrHS
        hrMS = hrMS
        gtHS = gtHS
        self.sr3.eval()
        with torch.no_grad():
            if isinstance(self.sr3, nn.DataParallel):
                result_SR = self.sr3.module.super_resolution(gtHS, hrMS, lrHS,SRF)
            else:
                result_SR = self.sr3.super_resolution(gtHS, hrMS, lrHS,SRF)
        self.sr3.train()
        return result_SR
...
if __name__ == "__main__":
    ...
    IN_CH_HSI = 144
    IN_CH_MSI = 4
    SRF = sio.loadmat("./srf_matrix.mat")
    SRF = SRF['SRF']


    train_datasat = Datasat('train', 160, IN_CH_HSI=IN_CH_HSI, IN_CH_MSI=IN_CH_MSI)
    train_loader = DataLoader(train_datasat, batch_size=BATCHSIZE, shuffle=True, num_workers=0, drop_last=False)
    ...
    sr3 = SR3(device, img_size=img_size, LR_size=LR_size, loss_type='l1',
              ...
              save_path='./model/Houston/LRHS_Elastic1000/',
              load_path='./model/Houston/LRHS_Elastic1000/SR3_model_epoch-499.pt', load=False,
              inner_channel=64,
